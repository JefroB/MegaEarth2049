// Add Minor NPCs Script for MegaEarth 2049
// This script adds all NPCs from minor_npcs.js to the game
...
// Import minor NPCs
const minorNPCs = require('./minor_npcs');
...
function writeNPCEventToFile(npcEvent, npcName) {
    const fileName = `minor_npc_${npcName.toLowerCase().replace(/\s+/g, '_').replace(/[^\w_]/g, '')}.json`;
...
// Main function to add all minor NPCs
async function addAllMinorNPCs() {
    console.log("Starting to add all minor NPCs to the game...");
...
// Run the script
addAllMinorNPCs().then(results => {
    console.log(`\nMinor NPC addition results:`);
